...
makeSuite('Pool: Simple FlashLoan', (testEnv: TestEnv) => {
  ...
  it('Check that reentrance borrow within flashloanSimple impacts rates', async () => {
    ...
    // Rates should have increased (the reentrant borrow raised utilization). The flash premium no
    // longer bumps the liquidity index, so the rate increase is the robust signal here.
